...
@onready var sprite_2d_2: AnimatedSprite2D = $Sprite2D/Sprite2D2
...
func _process(delta):
	...
	if is_on_floor():
		...
		if velocity.length() > 30:
			mult_x = velocity.normalized().x * .2
			sprite.scale = sprite.scale.lerp(Vector2(1.2, .8), delta * 5)
		...
		if abs(velocity.x) > 20 and !abs(velocity.y) > 400:
			sprite.rotation = lerp_angle(sprite.rotation, wrapf(velocity.normalized().angle(), -PI/2, PI/2) + (-.1 * velocity.x * delta), 2 * delta)
